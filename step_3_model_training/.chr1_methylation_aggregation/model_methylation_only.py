#!/usr/bin/env python3
"""
TissueBERT Model Architecture - METHYLATION ONLY VERSION
=========================================================

Key Change: Uses ONLY methylation embeddings, ignores DNA tokens.

Rationale: Logistic regression achieved 91% accuracy using only mean methylation
per region, proving the methylation signal alone is sufficient. The original
model summed DNA + methylation embeddings, which diluted the methylation signal.
...
class TissueBERT(nn.Module):
    """
    TissueBERT: Methylation-Only Transformer for Tissue Classification
    
    ===================================================================
    KEY CHANGE FROM ORIGINAL: Uses ONLY methylation embeddings
    ===================================================================
    
    Original model summed: dna_embeds + meth_embeds + pos_embeds
    This diluted the methylation signal (3 tokens vs 69 DNA tokens)
    
    New model uses: meth_embeds + pos_embeds (DNA ignored)
    
    Logistic regression proved methylation alone achieves 91% accuracy,
    so the DNA sequence is not needed for tissue classification.
    ===================================================================

    Architecture:
    1. Methylation status embedding (0=unmeth, 1=meth, 2=missing)
    2. Position embedding
    3. Transformer encoder (6 layers)
    4. Pooling + classification head

    Args:
        vocab_size (int): Size of DNA token vocabulary (kept for compatibility, not used)
    ...
    def __init__(self,
                 vocab_size=69,  # Kept for config compatibility, not used
                 hidden_size=512,
                 num_hidden_layers=6,
        ...
        # ===================================================================
        # METHYLATION-ONLY EMBEDDINGS
        # ===================================================================
        
        # Methylation status embedding (0=unmethylated, 1=methylated, 2=missing/no_CpG)
        # This is now the PRIMARY signal, not a secondary addition
        self.meth_embedding = nn.Embedding(3, hidden_size)

        # Position embedding - important for capturing regional patterns
        ...
        print("=" * 70)
        print("TissueBERT: METHYLATION-ONLY VERSION")
        print("=" * 70)
        print("This model uses ONLY methylation embeddings (ignores DNA tokens)")
        print("Embeddings: meth_embeds + pos_embeds")
        print("=" * 70)
    ...
    def forward(self, dna_tokens, methylation_status):
        """
        Forward pass - METHYLATION ONLY

        Args:
            dna_tokens: [batch_size, seq_length] - DNA token IDs (IGNORED)
            methylation_status: [batch_size, seq_length] - Methylation status (0, 1, 2)

        Returns:
            logits: [batch_size, num_classes] - Classification logits
        """
        batch_size, seq_length = methylation_status.shape

        # Create position indices
        position_ids = torch.arange(seq_length, dtype=torch.long, device=methylation_status.device)
        position_ids = position_ids.unsqueeze(0).expand(batch_size, -1)

        # ===================================================================
        # KEY CHANGE: Only use methylation embeddings, NOT DNA embeddings
        # ===================================================================
        meth_embeds = self.meth_embedding(methylation_status)
        pos_embeds = self.position_embedding(position_ids)

        # Combine embeddings (methylation + position only)
        ...
        # Create attention mask based on methylation status
        # Mask positions where methylation is "missing" (value 2) - OPTIONAL
        # For now, attend to all positions to let the model learn which matter
        # If you want to mask missing positions, uncomment:
        # attention_mask = (methylation_status != 2).unsqueeze(1).unsqueeze(2)
        # attention_mask = attention_mask.to(dtype=embeddings.dtype)
        # attention_mask = (1.0 - attention_mask) * -10000.0
        attention_mask = None  # Attend to all positions

        # Pass through transformer encoder
        hidden_states = self.encoder(embeddings, attention_mask)

        # Pool: mean pooling over VALID positions only
        # Mask out positions where methylation is "missing" (value 2)
        valid_mask = (methylation_status != 2).unsqueeze(-1).float()  # [batch, seq, 1]
        masked_hidden = hidden_states * valid_mask
        pooled = masked_hidden.sum(dim=1) / valid_mask.sum(dim=1).clamp(min=1.0)
...
def test_model():
    ...
    print("\nTesting TissueBERT (Methylation-Only) model...")
    ...
    # Print model info
    print(f"\nModel: TissueBERT (Methylation-Only)")
    ...
    dna_tokens = torch.randint(0, 65, (batch_size, seq_length))  # Will be ignored
    methylation = torch.randint(0, 3, (batch_size, seq_length))  # 0, 1, 2

    print(f"\nInput shapes:")
    print(f"  DNA tokens: {dna_tokens.shape} (IGNORED)")
    print(f"  Methylation: {methylation.shape} (USED)")
