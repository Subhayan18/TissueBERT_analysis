#!/usr/bin/env python3
"""
Stage 2 Blood-Masked Mixture Data Generator
============================================

This dataloader generates synthetic mixtures with BLOOD MASKED from labels.

Key differences from Stage 1:
- Input: Same mixed methylation (WITH blood component)
- Output: Blood-masked proportions (21 tissues, renormalized to 1.0)
- Training: Model learns to predict non-blood tissues from blood-dominant cfDNA

Strategy:
1. Generate Phase 3-style mixtures (60-100% blood)
2. Remove blood from proportion labels
3. Renormalize remaining tissues to sum to 1.0
4. Model sees blood signal but is trained to ignore it
...
class BloodMaskedMixtureDataset(Dataset):
    ...
    def _generate_bloodmasked_mixture(self) -> Dict[str, torch.Tensor]:
        ...
        if proportions_masked.sum() > 0:
            ...
            # Edge case: 100% blood → uniform distribution over non-blood
            proportions_masked = np.ones(self.n_tissues, dtype=np.float32) / self.n_tissues
