...
class TissueBERTDeconvolution(nn.Module):
    ...
    Process:
        ...
        4. Classifier → [batch, num_classes]
        5. Sigmoid activation → [batch, num_classes]
        6. Optional L1 normalization → [batch, num_classes] proportions
        
    Output normalization:
        - If normalize_output=True (default, Phase 3): Outputs sum to 1.0
        - If normalize_output=False (Stage 2): Outputs are absolute, sum to ~(1-blood_prop)
        
    Example Stage 2 usage:
        model = TissueBERTDeconvolution(num_classes=21, normalize_output=False)
        output = model(cfDNA_with_90pct_blood)
        # output: [0.05, 0.03, 0.02, ...] sums to 0.10 (not 1.0!)
    """
    ...
    def __init__(self,
                 ...
                 dropout=0.1,
                 n_regions=51089,
                 normalize_output=True):  # NEW: False for Stage 2 absolute labels
        super().__init__()

        self.num_classes = num_classes
        self.hidden_size = hidden_size
        self.n_regions = n_regions
        self.normalize_output = normalize_output  # NEW: Store normalization flag
    ...
    def forward(self, methylation_status, apply_renorm=None, renorm_strategy='threshold', renorm_params=None):
        ...
        Args:
            ...
            apply_renorm: If True, apply post-processing renormalization
                         If None/False, return raw proportions
        ...
        Returns:
            proportions: [batch, num_classes] tissue proportions
                        - If normalize_output=True: sum=1.0 (standard Phase 3)
                        - If normalize_output=False: sum<1.0 (Stage 2 absolute labels)
        """
        ...
        # L1 normalization (conditional based on normalize_output flag)
        if self.normalize_output:
            # Standard behavior (Phase 3): normalize to sum to 1.0
            proportions = sigmoid_outputs / sigmoid_outputs.sum(dim=1, keepdim=True)
        else:
            # Stage 2 absolute labels: no normalization, outputs can sum to <1.0
            # This allows model to predict true proportions like [0.05, 0.03, 0.02]
            # that sum to 0.10 (representing 10% non-blood tissue in 90% blood sample)
            proportions = sigmoid_outputs
